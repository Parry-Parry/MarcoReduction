...
DIM = 768 # Single BERT embedding

def truncate(array, sig):
    q = array[:, :DIM]
    pos = array[:, DIM:2*DIM]
...
def process_array(array, ablation='query', trunc=None) -> np.array:
    ...
    if ablation == 'trunc':
        ...
        return truncate(array, trunc)
    else:
        logging.error('ablation not recognised, continuing with full array')
        return array
...
def main(args):
    ...
    for k in args.k:
        for max_s in args.max_step:
            for t in args.t:
                model = Process(data, k, t, max_steps_per_sample=max_s)
                if args.start:
                    start_id = args.start 
                else:
